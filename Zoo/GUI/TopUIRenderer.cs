...
public class TopUIRenderer : IRenderer
{
    public GUIState Draw(GameController controller, int screenWidth, int screenHeight, Vector2 mousePos, bool isClicked, GUIState state)
    {
        ...
        Raylib.DrawText($"Pieniadze: {controller.MoneyController.Money}", 20, 100, 30, Color.DarkGreen);
        Raylib.DrawText($"Wynik: {controller.GameScore.CurrentScore} / Rekord: {controller.GameScore.MaxScore}", 20, 140, 30, Color.DarkGreen);
